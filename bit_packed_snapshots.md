...
Both Mongo storage and total retained JS memory include the dictionary.

| Mongo representation      | Retained JS representation        | Mongo Snappy KB | Mongo Zstd KB | Retained JS KB |
| ------------------------- | --------------------------------- | --------------: | ------------: | -------------: |
| Current objects           | Objects                           |           196.8 |         110.8 |          759.0 |
...
| Columnar / Binary arrays  | Adaptive typed columns            |           151.7 |          98.5 |          120.8 |
...
| **Bit-packed columns**    | **Three byte arrays + accessors** |       **123.0** |      **94.5** |      **103.2** |
...
Mongo results are from the earlier local MongoDB 8.3.3 experiment: 100 documents
per format, each with a distinct ID dictionary and the same numeric trace.
Every collection was compacted and its allocated `.wt` file size verified.
There were 27 collections (nine layouts times three compressors, including
uncompressed controls). Compaction used `freeSpaceTargetMB: 1`; all calls returned
`ok: 1, bytesFreed: 0`. After a locked flush, collection `storageSize` matched the
actual files. Compaction did not change the reported measurements and does not
change a collection's compressor. One repeated numerical trace with different
ID dictionaries is not a representative production sample.
Indexes, journals, replication and shared server overhead are excluded.
These are not Atlas measurements or production-wide savings estimates.

The implementation's three numeric columns were verified byte-for-byte against
the measured prototype on the full trace. The new version number has the same
BSON width, but Mongo was not rerun for the new version-2 documents: disk figures
above are the prior experiment, not a new version-2 disk measurement.

The implementation was benchmarked on Node 22.22.2 / V8 12.4.254.21-node.39,
